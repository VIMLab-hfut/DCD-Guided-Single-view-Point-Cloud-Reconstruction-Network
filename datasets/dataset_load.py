        ...
        for label in os.listdir(root):
            ...
            for item in os.listdir(c_path):
                ...
                for view in os.listdir(cc_path):
                    self.x.append(os.path.join(cc_path, view))
                #shuffle(views)
...
class ShapeNet55(Dataset):
    def __init__(self, root, split, category=None, transform=None, tgt_transform=None,
                 data_augment=False, number_of_view=6):
        self.root = root
        ...
        if not category:
            #raise NotImplementedError('Single Category Mode, target class must be specified')
            self.target_label = ['airplane', 'car', 'chair']
        else:
            self.target_label = [category]
        self.classes = self.target_label


        for label in self.target_label:
            config = os.path.join(root, '{}_{}.txt'.format(label, split))
            category_root = os.path.join(root, label)

            with open(config, 'r') as f:
                lines = f.readlines()
                for filename in lines:
                    filename = filename.strip()
                    item_path = os.path.join(os.path.join(category_root, filename), 'models')
                    ...
                    shuffle(views)
                    for view in views[:self.num_of_view]:
                        self.x.append(view)
    ...
    def __init__(self, root, category, split, transform=None, tgt_transform=None,
        ...
        with open(self.config, 'r') as f:
            ...
            for filename in lines:
                ...
                #
                #     self.y.append(npy_file)
                #     self.x.append(os.path.join(view_root, view))
