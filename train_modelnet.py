...
from datasets.dataset_load import MultiViewDataSet
from datasets.mv_dataset import PointDataSet
...
def main(conf):
    conf.name = 'no_curve_nmsl'.format(conf.cat)
    ...
    ds_loader_test = DataLoader(mv_ds_test, batch_size=conf.batch_size, drop_last=True)
    ...
    pc_ds = PointDataSet(root, ply_root, 'train', transform=_transform,
                         number_of_view=1)
    pc_loader = DataLoader(pc_ds, batch_size=10, drop_last=True, shuffle=True)
    ...
    for i in range(151):
        ...
        for idx, (multi_view, pc, _, _) in enumerate(ds_loader):
            ...
            mv = Variable(mv.cuda())  # Batch * 3 * 224 * 224
        ...
        if conf.sample_interval > 0 and i % opt.sample_interval == 0:
            with torch.no_grad():
                cd, emd = tdp_validate(model, ds_loader_test)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_interval', type=int, default=5, help='The gap between each sampling process')
